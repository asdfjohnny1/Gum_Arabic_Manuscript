...
pal2 <- colorRampPalette(c("#FF0000", "#00FF00", "#0000FF", "#FF00FF", "#FFFF00", "#00FFFF", "#FFA500", "#800080",
...
output_directory <- "~/Library/CloudStorage/OneDrive-UniversityofEastAnglia/PhD/Gum_Arabic/Gum Arabic shared folder/final_R/Graphs/Final_graphs/THE_final_graphs_for_justino"
```
...
ann_colors <- list(
  Tissue = c("Brain" = "#1B9E77", "Intestine" = "#D95F02")) # Set colors for Brain and Intestine)
  ...
  #####Column clustering
  # Create another heatmap with column clustering enabled
  heatmap_CoolVersion_JAA1 <- pheatmap::pheatmap(
    heatmap.matrix,
    legend_breaks = c(-3, 0, 3, max(heatmap.matrix)),
    # Define legend breaks
    legend_labels = c("Low", "Medium", "High", "Title\n"),
  ...
  print(heatmap_CoolVersion_JAA) # Heatmap without column clustering
  dev.off()  # Closes the current graphics device
  
  print(heatmap_CoolVersion_JAA1) # Heatmap with column clustering
...
standf <- function(x, t = total)
  round(t * (x / sum(x)))  # Function to standardize counts
...
phyloseq_rarefy@sam_data$gender_condition <- interaction(phyloseq_rarefy@sam_data$gender,
...
phyloseq_rarefy_rel_abundance_Ceto <- subset_taxa(phyloseq_rarefy_rel_abundance,
...
stat.test2 <- stat.test2 %>%
  ...
  mutate(
    xmin = case_when(group1 == "60% Control" ~ xmin - 0.2, # Adjust for specific comparison
                     TRUE ~ xmin),
    xmax = case_when(group1 == "60% Control" ~ xmax - 0.2, # Adjust for specific comparison
                     TRUE ~ xmax),
...
```{r phenotipic data, fig.show='hold'}
library(readxl)        # For reading Excel files
...
# annotations to add significance brackets 
annotation_df_GA <- data.frame(
  GA = "60",
  start = "GA\nFemale",
  end = "GA\nMale",
  label = "***"
...
#making a comparaison variable 
data$Category <- factor(interaction(data$Treatment, data$Sex),
                        levels = c("C.F", "C.M", "GA.F", "GA.M"),
                        labels = c("C\nFemale", "C\nMale",
                                   "GA\nFemale", "GA\nMale"))

### making clutch production plot 
clutch_JAA <- data %>%
  ggplot(mapping = aes(
    y = Clutch_production,
    x = Category
 ...
 facet_nested( ~ GA) +
  geom_signif(
    data = annotation_df_GA,
    aes(xmin = start, xmax = end, annotations = label, y_position = 1),
    textsize = 7, 
    vjust = -0.2,
    manual = TRUE,
    tip_length = 0,
...
annotation_df_weight <- data.frame(
  GA = "60",
  start = c("C\nFemale","GA\nFemale"),
  end = c("C\nMale","GA\nMale"),
  label = c("NS","NS")
...
weight_JAA <- data %>%
  ...
  geom_signif(
    data = annotation_df_weight,
    aes(xmin = start, xmax = end, annotations = label, y_position = 0.5),
    textsize = 7, 
    vjust = -0.2,
    manual = TRUE,
    tip_length = 0,
...
unfert_2hpf_JAA_log <- data %>%
  ...
  scale_x_discrete(name = NULL) +
  scale_y_continuous(name = "Log ratio of unfertilised eggs 2hpf (%)") +
  scale_fill_manual(
    values = saturation(pal5_JAA_blueberry.lemon.CAKE(2), scalefac(1.2)),
    labels = c("Female", "Male")
  ) +
 ...
 facet_nested( ~ GA) +
  geom_signif(
    data = annotation_df_GA,
    aes(xmin = start, xmax = end, annotations = label, y_position = -0.2),
    textsize = 7, 
    vjust = -0.2,
    manual = TRUE,
    tip_length = 0,
...
egg_JAA <- data %>%
  ...
  geom_signif(
    data = annotation_df_GA,
    aes(xmin = start, xmax = end, annotations = label, y_position = 250),
    textsize = 7, 
    vjust = -0.2,
    manual = TRUE,
    tip_length = 0,
...
# Define a secondary theme for plots where x-axis labels should be removed
custom_theme_no_x <- custom_theme + theme(
...
clutch_JAA.1 <- clutch_JAA + custom_theme_no_x
egg_JAA.1 <- egg_JAA + custom_theme_no_x
unfert_2hpf_JAA_nolegend.1 <- unfert_2hpf_JAA_log+ custom_theme
dead24hpf.1 <- dead24hpf + custom_theme

# Arrange and combine plots
phenotypic_patchwork_JAA_no <- (clutch_JAA.1 + egg_JAA.1) / 
                               (unfert_2hpf_JAA_nolegend.1 + dead24hpf.1) +
  plot_layout(guides = 'collect') +
  plot_annotation(tag_levels = 'A') &
  theme(plot.tag = element_text(size = 18, face = "bold"))


phenotypic_patchwork_JAA_no
...
dead24hpf <- data %>%
 ...
 facet_nested( ~ GA) +
  geom_signif(
    data = annotation_df_C,
    aes(xmin = start, xmax = end, annotations = label, y_position = 0.75),
    textsize = 7, 
    vjust = -0.2,
    manual = TRUE,
    tip_length = 0,
...
library(MASS)           # Functions and datasets to support Venables and Ripley’s MASS book
...
#############################################################################
#Weight difference
modelWeight.1 <- lmer((weight_after - weight_before) ~ GA_Treatment + Sex + weight_before +
                        (1 | Tank)  ,
                      data = data,
                      na.action = na.omit
)
modelWeight.3 <- lmer((weight_after - weight_before) ~ GA_Treatment + weight_before +
                        (1 | Tank)  ,
                      data = data,
                      na.action = na.omit
)
modelWeight.4 <- lmer((weight_after - weight_before) ~ GA_Treatment + (1 |
                                                                         Tank)  ,
                      data = data,
                      na.action = na.omit
)
modelWeight.5 <- lmer((weight_after - weight_before) ~ GA_Treatment + Sex +
                        (1 | Tank)  ,
                      data = data,
                      na.action = na.omit
)
modelWeight.6 <- lmer((weight_after - weight_before) ~ GA_Treatment * Sex +
                        (1 | Tank)  ,
                      data = data,
                      na.action = na.omit
)
modelWeight.7 <- lmer(((weight_after - weight_before) / weight_before) ~ GA_Treatment *
                        Sex + (1 | Tank)  ,
                      data = data,
                      na.action = na.omit
)
...
ggplot(data, aes(x = Block, y = Fertilised_nb_2h, col = Sex)) + geom_jitter() + geom_boxplot(alpha =
                                                                                               0.2) + facet_nested( ~ `2h_Counter`)
...
#6% GA
#how does it differ between counters
ggplot(data, aes(x = Block, y = Egg_nb, col = Sex)) + geom_jitter() + geom_boxplot(alpha =
                                                                                     0.2) + facet_nested( ~ `2h_Counter`)
...
#6%
#how does it differ between counters
ggplot(data, aes(x = Block, y = log_Abnormal_nb_2h, col = Sex)) + geom_jitter() + geom_boxplot(alpha =
                                                                                                 0.2) + facet_nested( ~ `2h_Counter`)
...
#6%
#how does it differ between counters
ggplot(data, aes(x = Block, y = Dead_nb_2h , col = Sex)) + geom_jitter() + geom_boxplot(alpha =
                                                                                          0.2) + facet_nested( ~ `2h_Counter`)
...
#6%
#how does it differ between counters
ggplot(data, aes(x = Block, y = Abnormal_nb_24h, col = Sex)) + geom_jitter() + geom_boxplot(alpha =
                                                                                              0.2) + facet_nested( ~ `2h_Counter`)
...
#6%
#how does it differ between counters
ggplot(data, aes(x = Block, y = Dead_nb_24h, col = Sex)) + geom_jitter() + geom_boxplot(alpha =
                                                                                          0.2) + facet_nested( ~ `2h_Counter`)
...
modeldistance.mm <- lmer(distance_mm ~ Treatment + (1 |
                                                      Block),
                         data = merged_data.5,
                         na.action = na.omit)
...
modeldistance.mm_2 <- lmer(distance_mm ~ Treatment + (1 |
                                                        Block) + (1 | ID),
                           data = merged_data.5,
                           na.action = na.omit)
...
modelspeed <- lmer(speed_mm_per_seconds ~ Treatment + (1 |
                                                         Block),
                   data = merged_data.5,
                   na.action = na.omit)
...
p_dist <- merged_data %>%
  ...
  scale_colour_manual(values = saturation(pal7.1(2), scalefac(1.2)),labels = c("Control", "60% Gum Arabic")) +
  theme_classic2() +
  geom_signif(
    data = annotation_dfdist,
    aes(xmin = start, xmax = end, annotations = label, y_position = 20000),
    textsize = 7, 
    vjust = -0.2,
    manual = TRUE,
    tip_length = 0,
...
p_speed<- merged_data %>%
  ...
  theme_classic2() +
  geom_signif(
    data = annotation_dfspeed,
    aes(xmin = start, xmax = end, annotations = label, y_position = 100),
    textsize = 7, 
    vjust = -0.2,
    manual = TRUE,
    tip_length = 0,
